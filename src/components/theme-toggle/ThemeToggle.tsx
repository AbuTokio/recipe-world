  ...
  return (
    <label className={`inline-flex items-center relative cursor-pointer scale-[${scale}]`}>
      ...
      <svg
        ...
        className={`absolute w-6 h-6 left-[13px] ${dark ? "fill-black opacity-60" : "fill-white opacity-100"}`}
        onClick={toggleTheme}>
      ...
      <svg
        ...
        id="Layer_1"
        xmlns="http://www.w3.org/2000/svg"
        className={`absolute w-6 h-6 right-[13px] ${dark ? "opacity-100 fill-white" : "opacity-60 fill-black"}`}
        onClick={toggleTheme}>
